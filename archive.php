...
<div class="well" role="main">
    ...
    <?php if (have_posts()) : ?>
        ...
        if (is_category()) { // If this is a category archive
            echo('<h1>' . __("Sorry, but there aren't any posts in the", "skeletor") . single_cat_title("", false) . ' category yet.</h1>');
        } elseif (is_date()) { // If this is a date archive
            echo('<h1>' . __("Sorry, but there aren't any posts with this date.", "skeletor") . '</h1>');
        } elseif (is_author()) { // If this is a category archive
            $userdata = get_userdatabylogin(get_query_var('author_name'));
            echo('<h1>' . __("Sorry, but there aren't any posts by ", "skeletor") . $userdata->display_name . ' yet.</h1>');
        } else {
            echo('<h1>' . __("No posts found", "skeletor") . '</h1>');
        }
        get_search_form();
    endif;
    ?>
